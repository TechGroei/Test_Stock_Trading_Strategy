...
# Import load_portfolio from trader to reuse the dual-loading logic (CSV + DB)
# Note: Ensure trader.py doesn't execute code on import (it uses if __name__ == "__main__")
from src.trader import load_portfolio
...
def fetch_current_prices(symbols):
    ...
    return yf.download(symbols, period="1d", progress=False, auto_adjust=False)["Adj Close"]

def evaluate():
    """Evaluate current portfolio performance with detailed metrics and save snapshot."""
    # Use shared load_portfolio to get data (logs DB/CSV mismatch if any)
    pf = load_portfolio()
    ...
    cash = config.START_CAPITAL - invested
    ...
    try:
        ...
        logger.error(f"Failed to fetch prices for performance evaluation: {e}")
        return None
    ...
    for sym in pf["symbol"]:
        yf_sym = sym.replace('.', '-')
        # Handle case where download might fail for specific symbol or return NaN
        if yf_sym not in prices.columns or np.isnan(prices[yf_sym].iloc[-1]):
            logger.warning(f"Using avg_price for {sym} as current price unavailable.")
    ...
    # Log summary
    logger.info("=== Portfolio Performance Snapshot ===")
    logger.info(f"Cash available: ${cash:.2f}")
    logger.info(f"Invested (book cost): ${invested:.2f}")
    logger.info(f"Market value of holdings: ${total_market_value:.2f}")
    logger.info(f"Total portfolio value: ${total_portfolio_value:.2f}")
    pct_pl = (total_unrealized_pl / invested * 100) if invested > 0 else 0
    logger.info(f"Total unrealized P/L: ${total_unrealized_pl:.2f} ({pct_pl:.2f}%)")
    logger.info(f"Number of positions held: {len(pf)}")
    logger.info("Individual holdings:")
    ...
    for sym in pf["symbol"]:
        ...
        logger.info(f" - {sym}: {shares:.4f} shares | Book: ${cost:.2f} | Market: ${mval:.2f} | P/L: ${pl:.2f}")

    winners = [s for s, v in unrealized_pl.items() if v > 0]
    losers = [s for s, v in unrealized_pl.items() if v < 0]
    logger.info(f"Winning positions: {len(winners)} | Losing positions: {len(losers)}")

    # Save a weekly snapshot CSV
    config.PERF_DIR.mkdir(exist_ok=True)
    now = datetime.utcnow()
    week_label = f"{now.year}-W{now.isocalendar().week:02d}"
    snapshot_file = config.PERF_DIR / f"performance_weekly_{week_label}.csv"
    ...
    logger.info(f"Saved weekly snapshot to {snapshot_file}")
...
def evaluate_weekly():
    ...
    config.PERF_DIR.mkdir(exist_ok=True)
    files = sorted(glob.glob(str(config.PERF_DIR / "performance_weekly_*.csv")))
    if not files:
        logger.info("No weekly snapshots found.")
        return
    ...
    for f in files:
        try:
            ...
            logger.error(f"Error reading snapshot {f}: {e}")
    ...
    logger.info("=== Weekly Portfolio Summary ===")
    # Convert to string for logging to avoid formatting issues
    logger.info("\n" + weekly_summary[["week_number","start_date","end_date","weekly_pl","win_loss"]].to_string())

    summary_file = config.BASE_DIR / "weekly_summary.csv"
